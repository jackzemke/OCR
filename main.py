...
def segment_characters(word_image, char_threshold=5, min_char_width=5):
    """
    Segments a single word into individual characters using vertical projection.
    Ensures the last character is handled properly.
    
    Args:
        word_image: Binary image of a single word.
        char_threshold: Maximum number of white pixels in a column to consider it a gap.
        min_char_width: Minimum width (in pixels) for a valid character.

    Returns:
        characters: List of binary images for each character.
        char_indices: List of (start_col, end_col) tuples for each character.
    """
    print("[INFO] Starting character segmentation...")

    # Compute vertical projection profile
    vertical_projection = np.sum(word_image == 255, axis=0)
    print(f"[DEBUG] Vertical projection profile computed. Length: {len(vertical_projection)}")

    # Detect character boundaries
    char_indices = []
    in_char = False
    char_start = 0  # Placeholder for current character start
    for col, pixel_sum in enumerate(vertical_projection):
        if pixel_sum > char_threshold and not in_char:
            # Start of a character
            char_start = col
            in_char = True
        elif pixel_sum <= char_threshold and in_char:
            # End of a character
            char_end = col
            if char_end - char_start >= min_char_width:  # Ignore noise
                char_indices.append((char_start, char_end))
                print(f"[DEBUG] Character detected from column {char_start} to {char_end}")
            in_char = False

    # Handle the last character if the loop ends while still in a character
    if in_char and (len(vertical_projection) - char_start) >= min_char_width:
        char_indices.append((char_start, len(vertical_projection)))
        print(f"[DEBUG] Last character detected from column {char_start} to {len(vertical_projection)}")

    # Extract characters
    characters = [word_image[:, start:end] for start, end in char_indices]
    print(f"[INFO] Total characters detected: {len(characters)}")

    return characters, char_indices
...
if __name__ == "__main__":
    ...
    for idx, line in enumerate(lines):
        ...
        words, word_indices = segment_words_by_spaces(line, space_threshold=5, min_space_width=100, min_word_width=20)

        for word_idx, word in enumerate(words):
            print(f"[INFO] Processing Word {word_idx + 1} in Line {idx + 1}...")
            
            # Segment characters within each word
            characters, char_indices = segment_characters(word)

            # Render characters side by side
            if len(char_indices) != 0:
                render_words_side_by_side(word, characters, f"Word {word_idx + 1} in Line {idx + 1}")
            else:
                print(f"[WARNING] No characters detected in Word {word_idx + 1}.")
